...
class Bird:
    ...
    def change(self):
        i,j=random.randrange(0,29),random.randrange(0,29)
        self.addr[i],self.addr[j]=self.addr[j],self.addr[i]
        self.upDate()
    #贪婪倒立变异
    def reverse(self):
        #随机选择一个城市
        cityx=random.randrange(1,30)
        noxcity=self.addr[:]
...
class Group:
    def __init__(self):
        self.groupSize=1000  #鸟的个数、粒子个数
        self.addrSize=30    #位置的维度，也就是TSP城市数量
        self.w=0.1 #w为惯性系数，也就是保留上次速度的程度
        self.pChange=0.1   #变异系数pChange
        self.pReverse=0.35   #贪婪倒立变异概率
        self.initBirds()
